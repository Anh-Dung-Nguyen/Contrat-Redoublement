...
test_that("test FONDA3", {
  for (ec in names(lesNotesEC_FONDAS3)){
    tests <- lesNotesEC_FONDAS3[[ec]]   # Get test names
    coefficients <- poids_FONDA_S3[[ec]]  # Get coefficients
    valid_tests <- !is.na(tests)
    tests <- tests[valid_tests]
    scores<-vector()
    for(test in tests){
      scores<-c(scores,LesUE[LesUE$UE == "FONDA S3" & LesUE$EC == ec, test])
    }
    scores<-as.numeric(scores)
    print(scores)
    real_average<-LesUE[LesUE$UE == "FONDA S3" & LesUE$EC == ec, "Moyenne"]
